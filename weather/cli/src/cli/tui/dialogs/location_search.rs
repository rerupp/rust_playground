    ...
    pub fn criteria_key_pressed(&mut self, key_event: KeyEvent) -> ControlFlow<DialogResult> {
        match self.criteria.key_pressed(key_event) {
            ControlFlow::Break(DialogResult::Exit) => {
                ...
                let location_criteria = self.criteria.as_location_criteria();
                match self.weather_data.search_locations(location_criteria) {
...
mod criteria {
    ...
    impl CriteriaDialog {
        ...
        /// Return the current search row as a location.
        ///
        pub fn as_location_criteria(&self) -> LocationCriteria {
            self.dialog.win().as_location_criteria()
        }
