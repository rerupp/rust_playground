...
impl HistoryRange {
    pub fn new(from: Date<Utc>, to: Date<Utc>) -> HistoryRange {
        HistoryRange { from, to }
    }
    /// Returns `true` if the *from* and *to* dates are equal.
    pub fn is_one_day(&self) -> bool { &self.from == &self.to }
    ...
    ///
    pub fn from_dates(dates: &Vec<Date<Utc>>) -> Vec<HistoryRange> {
...
mod history_tests {
    ...
    #[test]
    pub fn iterate() {
        let range = HistoryRange::new(Utc.ymd(2022, 6, 1),
                                      Utc.ymd(2022, 6, 30));
        let mut test_case = range.from.clone();
        let test_cases: Vec<Date<Utc>> = range.into_iter().collect();
        // let mut test_case = Utc.ymd(2022, 6, 1);
        assert_eq!(test_cases.len(), 30);
        for day in 0..30 {
            assert_eq!(test_cases[day], test_case);
    ...
    pub fn single_history_range() {
        let test_date = Utc.ymd(2022, 7, 6);
    ...
    pub fn multiple_history_range() {
        let test_dates = vec![
            Utc.ymd(2022, 7, 3),    // hr 1.lhs
            Utc.ymd(2022, 6, 30),   // hr 0.lhs
            Utc.ymd(2022, 7, 4),    // hr 1.rhs
            Utc.ymd(2022, 7, 1),    // hr 0.rhs
        ];
    ...
    pub fn to_iso8601_history_range() {
        let test_case = HistoryRange::new(Utc.ymd(2022, 7, 1),
                                          Utc.ymd(2022, 7, 2));
