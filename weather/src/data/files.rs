...
use super::{
    domain::{DailyHistory, DailyHistories, HistoryDates, HistoryRange, HistorySummary, Location, Locations},
};

use super::{DailyHistoryQuery, DataAPI, Error, Result, HistoryQuery, LocationQuery};
...
impl FsData {
    ...
    ///
    pub(crate) fn new_api(preferred_pathname: &str) -> Result<Box<dyn DataAPI>> {
...
///
// pub fn value_from_path(json_path: PathBuf) -> Result<Value> {
...
///
// pub fn value_from_reader<R: std::io::Read>(reader: R) -> Result<Value> {
...
mod archive {
    ...
    ///
    // pub fn date_from_name(name: &str) -> Result<Date<Utc>> {
    pub(crate) fn date_from_name(name: &str) -> Result<Date<Utc>> {
        if !name.ends_with(".json") {
            ...
            if name.len() < ymd_offset {
                ...
                if !ymd.chars().all(char::is_numeric) {
                    Err(Error::from("The name date was not all digits..."))
                } else {
                    ...
                    match Utc.ymd_opt(y, m, d) {
                        ...
                        _ => Err(Error::from(format!("to_date: '{}' month or day out of bounds...", ymd)))
                    }
                }
            }
        }
