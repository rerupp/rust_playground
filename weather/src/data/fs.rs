...
use crate::domain::{DailyHistory, HistoryDates, HistoryRange, HistorySummary, Location, Locations, log_error};

use super::{DailyHistories, DailyHistoryQuery, DataAPI, DataError, DataResult, HistoryQuery, LocationQuery};
...
/// The filesystem
impl FsData {
    /// Returns a new instance of the DataAPI.
    ///
    ...
    ///
    pub fn new_api(preferred_pathname: &str) -> DataResult<Box<dyn DataAPI>> {
        ...
        if !data_dir.is_dir() {
            ...
            Err(DataError::new(error_msg.as_str()))
        } else {
            Ok(Box::new(FsData { data_dir }))
        }
    }

    // fn data_dir(&self) -> &PathBuf { &self.data_dir }
}
...
impl DataAPI for FsData {
    ...
    ///
    fn get_daily_history(&self, query: DailyHistoryQuery) -> DataResult<DailyHistories> {
        ...
        for i in 0..reader.file_count() {
            ...
            if query.history_bounds.contains(&file_date) {
                ...
                if let Some(daily_history) = make_daily_history(&root, &query.location_id, file_date) {
                    ...
                    log_error(&format!("Error getting history for {}: {}...", &query.location_id, name));
                }
            }
        }
        Ok(DailyHistories {
    ...
    fn get_history_dates(&self, query: HistoryQuery) -> DataResult<HistoryDates> {
        ...
        Ok(HistoryDates {
            ...
            history_ranges: HistoryRange::from_dates(&dates),
        })
    }
    ...
    fn get_location_data(&self, location_query: LocationQuery) -> DataResult<Locations> {
        let include_location = |name: &String, alias: &String| -> bool {
            if location_query.filters.is_empty() {
                ...
                location_query.filters
                    .iter()
                    .find(|p| name_pat.starts_with(*p) || alias_pat.starts_with(*p))
                    .is_some()
            }
        };
        ...
        if !root.is_object() {
            Err(DataError::new("The locations root node is not an object..."))
        } else {
            let locations_value = root.get("locations");
            if locations_value.is_none() {
                Err(DataError::new("Did not find the array of locations..."))
            } else {
...
///
pub fn make_daily_history(root: &Value, location_id: &str, date: Date<Utc>) -> Option<DailyHistory> {
...
///
pub fn value_from_path(json_path: PathBuf) -> DataResult<Value> {
...
///
pub fn value_from_reader<R: std::io::Read>(reader: R) -> DataResult<Value> {
...
pub fn string_from_value(value: Option<&Value>) -> Option<String> {
    value.map_or(None, |v|
        v.as_str().map_or(None, |s| Some(s.to_string())))
}
...
// pub fn to_date(value: Option<&Value>, tz: &Tz) -> Option<Date<Tz>> {
//     to_i64(value).map_or(None, |t| Some(tz.timestamp(t, 0)))
// }

// #[inline]
// pub fn to_datetime(value: Option<&Value>, tz: &Tz) -> Option<DateTime<Tz>> {
//     to_i64(value).map_or(None, |t| Some(tz.timestamp(t, 0)))
// }
...
mod tests {
    ...
    fn from_value_all() {
        let mk_timestamp = |hour: u32| -> i64 {
            Utc.ymd(2022, 7, 15).and_hms(hour, 0, 0).timestamp()
        };
    ...
    fn string_converters() {
        ...
        let bad_string = json!(123);
        assert_eq!(string_from_value(Some(&bad_string)), None);
        assert_eq!(value_as_string(Some(&bad_string)), "".to_string());
    }

    #[test]
    pub fn good_path() {
        assert!(FsData::new_api(".").is_ok());
    }

    #[test]
    pub fn bad_path() {
        assert!(FsData::new_api("./something_that_should_not_exist").is_err());
    }
}
...
mod archive {
    ...
    impl Reader {
        ...
        /// # Arguments
        ///
        /// * `file_number` - the file index withingthe **ZIP** archive.
        ///
        #[inline]
        pub fn get(&mut self, file_number: usize) -> DataResult<ZipFile> {
            Ok(self.zip_archive.by_index(file_number)?)
        }
    }
    ...
    ///
    pub fn date_from_name(name: &str) -> DataResult<Date<Utc>> {
        if !name.ends_with(".json") {
            Err(DataError::new("Expected name to end in '.json'..."))
        } else {
            let ymd_offset = "yyyymmdd.json".len();
            if name.len() < ymd_offset {
                Err(DataError::new("The format of the name is incorrect..."))
            } else {
                let ymd_index = name.len() - ymd_offset;
                let ymd: &str = &name[ymd_index..ymd_index + 8];
                if !ymd.chars().all(char::is_numeric) {
                    Err(DataError::new("The name date was not all digits..."))
                } else {
                    let y = ymd[..4].parse().unwrap();
                    let m = ymd[4..6].parse().unwrap();
                    let d = ymd[6..].parse().unwrap();
                    match Utc.ymd_opt(y, m, d) {
                        LocalResult::Single(d) => Ok(d),
                        _ => Err(DataError::new(&format!("to_date: '{}' month or day out of bounds...", ymd)))
                    }
                }
            }
        }
    ...
    mod tests {
        ...
        pub fn parse_date_from_name() {
            assert_eq!(Utc.ymd(2020, 4, 29), date_from_name("mesa/mesa-20200429.json").unwrap());
            assert_eq!(Utc.ymd(2022, 10, 31), date_from_name("020221031.json").unwrap());
        }
