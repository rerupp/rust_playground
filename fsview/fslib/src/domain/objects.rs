...
impl Metadata {
    ...
    /// Checks if the metadata is a folder and has children.
    pub fn has_childen(&self) -> bool {
        match self {
            Metadata::Root(folder_md) | Metadata::Folder(folder_md) => folder_md.children.len() > 0,
            _ => false,
        }
    }
}
...
pub struct FolderMd {
    ...
    pub children: BTreeMap<String, Metadata>,
}
...
pub(crate) struct DuplicateIds {
    /// The duplicate file filename.
    pub filename: String,
    /// The parent folder identifiers and associated file identifiers for a duplicate filename.
    pub ids: Vec<(i64, i64)>,
}
impl DuplicateIds {
    /// Create the metadata for a filename that's in multiple folders.
    ///
    /// # Arguments
    ///
    /// `filename` is the name of the duplicate file.
    pub fn new(filename: &str) -> Self {
        DuplicateIds { filename: String::from(filename), ids: vec![] }
    }
    /// Add the parent and file identifiers for the duplicate filename.
    ///
    /// # Arguments
    ///
    /// * `parent_id` is the folder identifier.
    /// * `file_id` is the file identifier.
    pub fn add(&mut self, parent_id: i64, file_id: i64) -> &mut Self {
        self.ids.push((parent_id, file_id));
        self
    }
}
