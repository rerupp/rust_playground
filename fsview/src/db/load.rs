...
fn insert_folders(tx: &sql::Transaction, folder_md: &FolderMetadata, parent_id: i64) -> Result<InsertCount> {
    ...
    match tx.execute(FOLDERS_INSERT, params) {
        Err(error) => Err(Error(format!("directory='{}' {error}.", folder_md.pathname()))),
        _ => {
...
fn insert_files(tx: &sql::Transaction, file_md: &FileMetadata, parent_id: i64) -> Result<InsertCount> {
    #[rustfmt::skip]
    log::trace!("{1}: {0}", file_md.pathname(), if file_md.is_symlink {
            "SYMLINK"
    ...
    match tx.execute(FILES_INSERT, params) {
        // Err(error) => Err(DbError::Internal(format!("File='{}' {}.", file_md.pathname(), error))),
        Err(error) => Err(Error(format!("file='{}' {error}.", file_md.pathname()))),
        _ => Ok(ADD_FILE),
    }
}
...
fn insert_problems(tx: &sql::Transaction, problem_md: &ProblemMetadata, parent_id: i64) -> Result<InsertCount> {
    ...
    match tx.execute(PROBLEMS_INSERT, params) {
        // Err(error) => Err(DbError::Internal(format!("Problem='{}' {}.", problem_md.pathname(), error))),
        Err(error) => Err(Error(format!("db: problem='{}' {error}.", problem_md.pathname()))),
        _ => Ok(ADD_PROBLEM),
    }
}
...
const ADD_FOLDER: InsertCount = InsertCount {
    folders: 1,
    files: 0,
    problems: 0,
    empty_files: 0,
...
const ADD_FILE: InsertCount = InsertCount {
    folders: 0,
    files: 1,
    problems: 0,
    empty_files: 0,
...
/// Used when a file is added to the database to update the [InsertCount].
const ADD_PROBLEM: InsertCount = InsertCount {
    folders: 0,
    files: 0,
    problems: 1,
    empty_files: 0,
