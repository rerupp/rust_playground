...
class DateRangeSelector:
    def __init__(self, parent, history_dates: List[DateRange], date_range: Optional[DateRange] = None,
        ...
        if date_range:
            ...
            self._current_date_range = DateRange(start, end)
    ...
    def _initialize_histories_combobox(self):
        if not self._history_dates:
            ...
            def fmt_date_range(dr: DateRange) -> str:
                if dr.start == dr.end:
                    return fmt(dr.start)
    ...
    def set_history_dates(self, history_dates: List[DateRange], selected_date_range: DateRange):
        if self._history_selector:
