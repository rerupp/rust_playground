...
from ..domain import DateRange, LocationHistoryDates, WeatherData
...
class HistoriesDates(WeatherView):
    def __init__(self, parent, weather_data: WeatherData, add_tab: Callable[[str, WeatherView], None],
        ...
        self._locations_history_dates: List[LocationHistoryDates] = []
        self.refresh()
    ...
    def refresh(self):
        try:
            elapsed = Stopwatch()
            self._locations_history_dates = self._weather_data.get_history_dates()
...
class HistoryDatesView(tk.Frame):
    ...
    def refresh(self, locations_history_dates: List[LocationHistoryDates]):
        """
        ...
        history_date_rows = [HistoryDatesRow(lhd) for lhd in locations_history_dates]
        ...
        for history_dates_row in history_date_rows:
            ...
            if history_dates_row.history_dates:
                max_date_range_width = max([measure(dr) for dr in history_dates_row.history_dates])
            else:
...
class HistoryDatesRow:
    ...
    def __init__(self, location_history_dates: LocationHistoryDates):
        self.iid = location_history_dates.location.alias
        self.name = location_history_dates.location.name
        self.history_dates = [HistoryDatesRow.__describe_date_range(dr) for dr in location_history_dates.history_dates]
    ...
    @staticmethod
    def __describe_date_range(date_range: DateRange) -> str:
