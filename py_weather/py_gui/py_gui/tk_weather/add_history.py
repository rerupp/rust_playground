...
class AddHistory:
    ...
    def _get_history(self):
        try:
            history_client = self._weather_data.get_history_client()
            ...
            if not get_history.is_canceled:
                ...
                self._weather_data.add_histories(daily_histories)
...
class DateSelector(Dialog):
    def __init__(self, parent, location: Location, date_range: DateRange, history_dates: List[DateRange]):
        self._location = location
...
class GetHistory(Dialog):
    def __init__(self, parent, location: Location, history_client: HistoryClient):
